...
function renderChatTab(el, comp) {
    ...
    const doSend = async () => {
        ...
        const payload = buildChatPayload(comp);
        let full = '';
...
function buildChatPayload(comp) {
    // 查找连接的 Memory 组件获取消息历史
    const memComp = findConnectedMemory(comp.id);
    const messages = memComp ? memComp.messages : (comp.messages || []);

    // ── 意图检测：用户是否明确要求 Office 操作 ──
    // 注意：正则必须足够精确，避免日常用语误触发（如"列出""列表""报告一下"等）
    const lastUserMsg = [...messages].reverse().find(m => m.role === 'user');
    const userText = (lastUserMsg?.content || '').toLowerCase();
    const userWants = {
        excel: /(?:创建|生成|导出|下载|保存|写|做|制作|新建|弄).*(?:excel|表格|xlsx|工作表|电子表格|spreadsheet)|(?:excel|表格|xlsx|工作表|电子表格|spreadsheet).*(?:创建|生成|导出|下载|保存|写|做|制作|新建|弄)|\.xlsx|新建.*(?:工作簿|workbook)/.test(userText),
        word:  /(?:创建|生成|导出|下载|保存|写|做|制作|新建|起草|编写).*(?:word|文档|docx|报告书)|(?:word|文档|docx|报告书).*(?:创建|生成|导出|下载|保存|写|做|制作|新建|起草|编写)|\.docx|新建.*文档/.test(userText),
        ppt:   /(?:创建|生成|导出|下载|保存|做|制作|新建).*(?:ppt|powerpoint|演示文稿|幻灯片|pptx|presentation)|(?:ppt|powerpoint|演示文稿|幻灯片|pptx|presentation).*(?:创建|生成|导出|下载|保存|做|制作|新建)|\.pptx|新建.*演示/.test(userText),
    };

    const payload = {
        messages: messages,
        model: comp.apiSettings.model || undefined,
        api_base: comp.apiSettings.apiBase || undefined,
        api_key: comp.apiSettings.apiKey || undefined,
        max_tool_rounds: comp.apiSettings.maxToolRounds || 50,
    };

    // 最大搜索轮数：优先从组件读取，其次 localStorage，默认 10
    const savedRounds = safeStorage.get('wybzd-max-search-rounds');
    payload.max_search_rounds = (savedRounds != null) ? savedRounds : 10;
    const wsComp = STATE.components.find(c => c.type === 'web_search');
    if (wsComp && typeof wsComp.maxSearchRounds === 'number' && wsComp.maxSearchRounds >= 1) {
        payload.max_search_rounds = wsComp.maxSearchRounds;
    }

    // 收集连线模块的增强能力
    const fnTools = [];       // Function Calling 工具定义
    const toolNames = [];     // tool_registry 中的工具名
    ...
    STATE.connections.filter(c => c.sourceCompId === comp.id).forEach(c => {
        ...
        switch (tgt.type) {
            case 'memory':
                // Memory 组件只用于消息存储，不作为 tool
                break;
            case 'memory_summarizer':
                addTools('memory_summarize');
                break;
            case 'system_prompt':
                if (tgt.activePromptContent) payload.system_prompt = tgt.activePromptContent;
                break;
            case 'skill_document':
            case 'skill_frontend':
            case 'skill_uiux':
            ...
            case 'skill_pua':
                if (tgt.toolEnabled !== false) {
                    ...
                    const sid = skillIdMap[cd.type];
                    if (sid) {
                        const cachedPrompt = SKILL_PROMPT_CACHE[sid];
                        if (cachedPrompt) {
                ...
                break;
            case 'sequential_executor':
                // LLM 连接到顺序执行器 → 从顺序执行器收集有序工具
                const seqNames = collectSequentialTools(tgt.id);
                if (seqNames.length > 0) {
                    // 有序工具覆盖无序工具
                    toolNames.length = 0;
                    toolNames.push(...seqNames);
                    // 注入顺序约束到 system prompt
                    const seqConstraint = buildSequentialConstraintText(tgt);
                    if (seqConstraint) {
                        payload.system_prompt = (payload.system_prompt || '') + seqConstraint;
                    }
                }
                break;
            case 'agent':
                // Agent 编排器 — 收集其输出端口连接的工具
                if (tgt.toolEnabled !== false) {
                    const agentToolNames = collectAgentToolNames(tgt.id);
                    if (agentToolNames.length > 0) {
                        agentToolNames.forEach(n => { if (!toolNames.includes(n)) toolNames.push(n); });
                        const agentHint = buildAgentHintText(tgt);
                        if (agentHint) {
                            payload.system_prompt = (payload.system_prompt || '') + agentHint;
                        }
                    }
                }
                break;
            case 'executor':
                // LLM 连接到执行器 → 告诉 LLM 执行器上有哪些工具，让 LLM 直接调用
                if (tgt.toolEnabled !== false) {
                    const execToolNames = collectExecutorToolNames(tgt.id);
                    if (execToolNames.length > 0) {
                        // 把执行器上的工具加入可用列表
                        execToolNames.forEach(n => { if (!toolNames.includes(n)) toolNames.push(n); });
                        // 注入提示告知 LLM 这些工具的来源和执行方式
                        const execHint = buildExecutorHintText(tgt);
                        if (execHint) {
                            payload.system_prompt = (payload.system_prompt || '') + execHint;
                        }
                    }
                }
                break;
            case 'skill_auto_call':
                // 技能自动调用：智能模式下 LLM 自主选择技能
                if (tgt.toolEnabled !== false) {
                    // 查找下游 skills_manager 获取技能列表
                    const skmConn = STATE.connections.find(
                        cc => cc.sourceCompId === tgt.id && cc.sourcePortId === 'auto-skm-out'
                    );
                    if (skmConn) {
                        const skm = STATE.components.find(x => x.id === skmConn.targetCompId);
                        if (skm && skm.type === 'skills_manager') {
                            refreshSkillsManagerSkills(skm);
                            const skillIds = (skm.skmSkills || []).map(s => s.skillId).filter(Boolean);
                            if (skillIds.length > 0) {
                                if (tgt.smartMode !== false) {
                                    // 智能模式：添加 use_skill 工具让 LLM 自行选择
                                    const skillList = skillIds.map(sid => {
                                        const name = SKILL_PROMPT_CACHE[sid + '_name'] || sid;
                                        const desc = (SKILL_PROMPT_CACHE[sid + '_desc'] || '').slice(0, 100);
                                        return `  - ${sid}: ${desc}`;
                                    }).join('\n');
                                    payload.system_prompt = (payload.system_prompt || '') +
                                        '\n\n## 可用技能（智能模式）\n\n' +
                                        '你拥有以下可用技能。调用 `use_skill` 工具来激活某个技能，激活后会收到该技能的完整指导。\n' +
                                        '你可以根据任务需求决定是否调用技能、调用哪个技能。\n\n' +
                                        skillList;
                                    fnTools.push({
                                        type: 'function',
                                        function: {
                                            name: 'use_skill',
                                            description: '激活一个技能以获得专业指导。传入技能 ID，系统会返回该技能的完整 System Prompt。根据任务需求自主决定是否调用。',
                                            parameters: {
                                                type: 'object',
                                                properties: {
                                                    skill_id: {
                                                        type: 'string',
                                                        description: '要激活的技能 ID。可选值：' + skillIds.join(', '),
                                                        enum: skillIds,
                                                    },
                                                },
                                                required: ['skill_id'],
                                    ...
                                    // 普通模式：全部注入（同 skills_manager 直连）
                                    const prompts = skillIds
                                        .map(sid => SKILL_PROMPT_CACHE[sid])
                                        .filter(Boolean);
                                    if (prompts.length > 0) {
                                        payload.system_prompt = (payload.system_prompt || '') + '\n\n## 已激活技能\n\n' + prompts.join('\n\n---\n\n');
                                    }
                                    skillIds.forEach(sid => {
                                        const s = SKILL_PROMPT_CACHE[sid + '_tools'];
                                        if (s) s.forEach(t => { if (!toolNames.includes(t)) toolNames.push(t); });
                                    });
                                }
                            }
                        }
                ...
                break;
            case 'skills_manager':
                // 技能管理器连接 -> 收集所有已连接技能的 System Prompt 合并注入
                if (tgt.toolEnabled !== false) {
                    // 检查是否有上游 skill_auto_call 已处理（避免重复注入）
                    const upstreamAuto = STATE.connections.find(
                        cc => cc.targetCompId === tgt.id && cc.targetPortId === 'skm-llm-in' &&
                              STATE.components.find(x => x.id === cc.sourceCompId)?.type === 'skill_auto_call'
                    );
                    if (upstreamAuto) break;  // 已由 skill_auto_call 处理
                    refreshSkillsManagerSkills(tgt);
                    const skillIds = (tgt.skmSkills || []).map(s => s.skillId);
                    if (skillIds.length > 0) {
                        // 从缓存获取各技能的 prompt 并合并
                        const prompts = skillIds
                            .map(sid => SKILL_PROMPT_CACHE[sid])
                            .filter(Boolean);
                        if (prompts.length > 0) {
                        ...
                        skillIds.forEach(sid => {
                            const s = SKILL_PROMPT_CACHE[sid + '_tools'];
                            if (s) {
                                s.forEach(t => { if (!toolNames.includes(t)) toolNames.push(t); });
                            }
                        });
                    }
                }
            ...
            case 'plan':
                // Plan 模块连接：添加计划相关工具
                if (tgt.toolEnabled !== false) {
                    toolNames.push('plan_generate', 'plan_execute_step');
                }
                break;
            case 'function_calling':
                if (tgt.tools && tgt.tools.length > 0) {
                    fnTools.push(...tgt.tools.map(t => ({
                        type: 'function',
            ...
            case 'json_mode':
                if (tgt.jsonSchema) {
                    payload.response_format = {
                        type: 'json_schema',
                        json_schema: { name: 'response', strict: true, schema: tgt.jsonSchema },
                    };
                }
                break;
            // 工具模块：只传已开启（toolEnabled !== false）的工具
            case 'web_search':
                if (tgt.toolEnabled !== false) toolNames.push('web_search');
                break;
            case 'calculator':
                if (tgt.toolEnabled !== false) toolNames.push('calculator');
                break;
            case 'code_executor':
                if (tgt.toolEnabled !== false) toolNames.push('code_executor');
                break;
            case 'text_tools':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('text_analyze');
                    toolNames.push('text_format');
                }
                break;
            case 'time_query':
                if (tgt.toolEnabled !== false) toolNames.push('get_current_time');
                break;
            case 'url_fetch':
                if (tgt.toolEnabled !== false) toolNames.push('url_fetch');
                break;
            case 'file_ops':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('file_read', 'file_write', 'glob_search', 'grep_search', 'file_edit');
                }
                break;
            case 'json_query':
                if (tgt.toolEnabled !== false) toolNames.push('json_query');
                break;
            case 'vector_memory':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('embeddings_search', 'embeddings_index');
                }
                break;
            // --- MCP Office 工具（仅在用户明确要求时才激活）---
            case 'mcp_word':
                if (tgt.toolEnabled !== false && userWants.word) {
                    toolNames.push('word_create', 'word_add_heading', 'word_add_paragraph',
                                   'word_add_table', 'word_save');
                }
                break;
            case 'mcp_excel':
                if (tgt.toolEnabled !== false && userWants.excel) {
                    toolNames.push('excel_create', 'excel_write_cell', 'excel_read_cell',
                                   'excel_add_sheet', 'excel_save');
                }
                break;
            case 'mcp_ppt':
                if (tgt.toolEnabled !== false && userWants.ppt) {
                    toolNames.push('ppt_create', 'ppt_add_slide', 'ppt_add_text',
            ...
            case 'mcp_weather':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('weather_current', 'weather_forecast');
                }
                break;
            case 'mcp_database':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('db_query', 'db_list_tables', 'db_schema');
                }
                break;
            case 'mcp_git':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('git_status', 'git_log', 'git_diff', 'git_branch');
                }
                break;
            case 'mcp_clipboard':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('clipboard_read', 'clipboard_write');
                }
                break;
            case 'mcp_encoding':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('base64_encode', 'base64_decode', 'hash_compute', 'password_generate', 'uuid_generate', 'regex_test', 'diff_text', 'markdown_to_html', 'html_to_markdown', 'unit_convert', 'url_encode');
                }
                break;
            case 'mcp_system':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('system_info', 'dns_lookup', 'qr_generate', 'open_file', 'desktop_notify');
                }
                break;
            case 'mcp_email':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('email_send');
                }
                break;
            case 'mcp_translate':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('translate_text', 'detect_language');
                }
                break;
            case 'mcp_calendar':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('calendar_list', 'calendar_create');
                }
                break;
            case 'mcp_pdf':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('pdf_read', 'pdf_create', 'pdf_merge');
                }
                break;
            case 'mcp_finance':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('currency_convert', 'stock_price');
                }
                break;
            case 'mcp_geocode':
                if (tgt.toolEnabled !== false) {
                    toolNames.push('geocode_address', 'reverse_geocode', 'ip_geolocation', 'distance_calc');
                }
                break;
            case 'mcp_navigation':
                if (tgt.toolEnabled !== false) {
    ...
    if (fnTools.length > 0) {
        payload.tools = fnTools;
    }

    // 智能模式：skill_auto_call 连接时设置 smart_mode 并填充 available_skills
    const autoCallComp = STATE.components.find(c => c.type === 'skill_auto_call' && c.toolEnabled !== false);
    if (autoCallComp && autoCallComp.smartMode !== false) {
        payload.smart_mode = true;
        // 查找关联的 skills_manager 收集可用技能
        const skmConn = STATE.connections.find(
            cc => cc.sourceCompId === autoCallComp.id && cc.sourcePortId === 'auto-skm-out'
        );
        if (skmConn) {
            const skm = STATE.components.find(x => x.id === skmConn.targetCompId);
            if (skm && skm.type === 'skills_manager') {
...
// ── 生成执行器提示文本（告诉 LLM 它有哪些工具）──
function buildExecutorHintText(execComp) {
    const toolNames = collectExecutorToolNames(execComp.id);
    ...
    return (
        ...
        `你可以直接调用以下工具来完成任务：\n\n` +
        `${toolList}\n\n` +
        `这些工具已连接到执行器，调用后执行器会按顺序执行。` +
        `根据任务需要自由选择调用哪些工具，可以调用一个或多个。\n`
    );
}

// ── 从 Agent 编排器收集工具名称 ──
function collectAgentToolNames(agentCompId) {
    const toolPorts = ['agent-tool-1', 'agent-tool-2', 'agent-tool-3', 'agent-tool-4', 'agent-tool-5'];
    const names = [];
    toolPorts.forEach(portId => {
        const conn = STATE.connections.find(
            c => c.sourceCompId === agentCompId && c.sourcePortId === portId
        );
        if (conn) {
            const tgt = STATE.components.find(x => x.id === conn.targetCompId);
            if (tgt) {
...
// ── 生成 Agent 编排器提示文本 ──
function buildAgentHintText(agentComp) {
    const toolNames = collectAgentToolNames(agentComp.id);
    if (toolNames.length === 0) return '';
    ...
    return (
        `\n\n## 🤖 Agent 编排器已激活\n` +
        `你是一个完整的 AI Agent，自动执行 Plan → Execute → Reflect 循环。\n\n` +
        `可用工具:\n${toolList}\n\n` +
        `工作流程:\n` +
        `1. 分析用户任务，制定执行计划\n` +
        `2. 逐步执行，调用合适的工具\n` +
...
// ── 生成顺序约束文本 ──
function buildSequentialConstraintText(seqComp) {
    const ordered = collectSequentialTools(seqComp.id);
    if (ordered.length === 0) return '';

    const strict = seqComp.strictMode !== false;
    const sequence = ordered.map((n, i) => `${i + 1}. \`${n}\``).join('\n');

    if (strict) {
        return (
            `\n\n## ⚠️ 顺序执行约束（必须严格遵守）\n` +
            `你必须严格按照以下顺序依次调用工具，不得跳过或并行调用：\n\n` +
            `${sequence}\n\n` +
            `规则:\n` +
            `1. 只能从第 1 步开始，按序号依次调用\n` +
            `2. 每步完成后再进行下一步\n` +
            `3. 不允许同时调用多个工具\n` +
            `4. 不允许跳过任何步骤\n` +
            `5. 如某步失败则停止并报告\n`
...
const AGENT_PRESETS = {
    ...
    search: {
        name: '🔍 搜索助手',
        description: 'LLM + Agent + Web Search + URL Fetch + Text Tools + Memory',
        components: [
            { type: 'llm', size: 5, x: 3, y: 3, apiSettings: { apiBase: '', apiKey: '', model: '', provider: '自定义' } },
        ...
        connections: [
            ...
            { source: 1, sourcePort: 'agent-tool-3', target: 4, targetPort: 'tt-in' },
    ...
    analyst: {
        name: '📊 数据分析师',
        description: 'LLM + Agent + Code Executor + Excel + Calculator + FileOps + Memory',
        components: [
            { type: 'llm', size: 5, x: 3, y: 3, apiSettings: { apiBase: '', apiKey: '', model: '', provider: '自定义' } },
            { type: 'agent', size: 6, x: 3, y: 35 },
        ...
        connections: [
            ...
            { source: 8, sourcePort: 'mem-out', target: 1, targetPort: 'agent-mem-in' },
        ],
    },
    plan_exec: {
        name: '🧠 计划-执行 Agent',
    ...
    doc_pro: {
        name: '📝 文档专家',
        description: 'LLM + Skills Manager(Document Skill) + Word + Search + PDF + Memory',
        components: [
            { type: 'llm', size: 5, x: 3, y: 3, apiSettings: { apiBase: '', apiKey: '', model: '', provider: '自定义' } },
            { type: 'skills_manager', size: 5, x: 3, y: 35 },
            { type: 'skill_document', size: 4, x: 55, y: 5, toolEnabled: true },
            { type: 'web_search', size: 3, x: 70, y: 5, toolEnabled: true },
            { type: 'mcp_word', size: 3, x: 55, y: 38, toolEnabled: true },
            { type: 'mcp_pdf', size: 3, x: 70, y: 38, toolEnabled: true },
            { type: 'memory', size: 3, x: 3, y: 73 },
        ],
        connections: [
            { source: 0, sourcePort: 'llm-out', target: 1, targetPort: 'skm-llm-in' },
            { source: 1, sourcePort: 'skm-skill-1', target: 2, targetPort: 'skill-in' },
            { source: 0, sourcePort: 'llm-out', target: 3, targetPort: 'ws-in' },
            { source: 0, sourcePort: 'llm-out', target: 4, targetPort: 'word-in' },
            { source: 0, sourcePort: 'llm-out', target: 5, targetPort: 'pdf-in' },
            { source: 6, sourcePort: 'mem-out', target: 0, targetPort: 'llm-mem-in' },
        ],
    },
    design_pro: {
        name: '🎨 设计专家',
        description: 'LLM + Skills Manager(Frontend + UI/UX Pro Max) + Web Search + Memory',
        components: [
            { type: 'llm', size: 5, x: 3, y: 3, apiSettings: { apiBase: '', apiKey: '', model: '', provider: '自定义' } },
        ...
        connections: [
            { source: 0, sourcePort: 'llm-out', target: 1, targetPort: 'skm-llm-in' },
            { source: 1, sourcePort: 'skm-skill-1', target: 2, targetPort: 'skill-in' },
            { source: 1, sourcePort: 'skm-skill-2', target: 3, targetPort: 'skill-in' },
            { source: 0, sourcePort: 'llm-out', target: 4, targetPort: 'ws-in' },
            { source: 0, sourcePort: 'llm-out', target: 5, targetPort: 'uf-in' },
            { source: 6, sourcePort: 'mem-out', target: 0, targetPort: 'llm-mem-in' },
        ],
    },
    skill_forge: {
        name: '⚡ 技能工坊',
        description: 'LLM + Skills Manager(Find + Creator + Superpowers) + Web Search + Memory',
        components: [
            { type: 'llm', size: 5, x: 3, y: 3, apiSettings: { apiBase: '', apiKey: '', model: '', provider: '自定义' } },
            { type: 'skills_manager', size: 5, x: 3, y: 38 },
        ...
        connections: [
            ...
            { source: 1, sourcePort: 'skm-skill-1', target: 2, targetPort: 'skill-in' },
            { source: 1, sourcePort: 'skm-skill-2', target: 3, targetPort: 'skill-in' },
            { source: 1, sourcePort: 'skm-skill-3', target: 4, targetPort: 'skill-in' },
    ...
    dev_kit: {
        name: '🛠️ 开发工具包',
        description: 'LLM + Agent + Git + Database + Code + System + Encoding + FileOps + Memory',
        components: [
            { type: 'llm', size: 5, x: 3, y: 3, apiSettings: { apiBase: '', apiKey: '', model: '', provider: '自定义' } },
            { type: 'agent', size: 6, x: 3, y: 35 },
    ...
    research: {
        name: '🔬 深度研究',
        description: 'LLM + Skills Manager(Find Skills) + Agent + Search + URL Fetch + PDF + Translate + Memory',
        components: [
            { type: 'llm', size: 6, x: 3, y: 3, apiSettings: { apiBase: '', apiKey: '', model: '', provider: '自定义' } },
        ...
        connections: [
            { source: 0, sourcePort: 'llm-out', target: 1, targetPort: 'skm-llm-in' },
            { source: 1, sourcePort: 'skm-skill-1', target: 2, targetPort: 'skill-in' },
            { source: 0, sourcePort: 'llm-out', target: 3, targetPort: 'agent-llm-in' },
            { source: 3, sourcePort: 'agent-tool-1', target: 4, targetPort: 'ws-in' },
            { source: 3, sourcePort: 'agent-tool-2', target: 5, targetPort: 'uf-in' },
            { source: 3, sourcePort: 'agent-tool-3', target: 6, targetPort: 'pdf-in' },
            { source: 3, sourcePort: 'agent-tool-4', target: 7, targetPort: 'tr-in' },
            { source: 8, sourcePort: 'mem-out', target: 0, targetPort: 'llm-mem-in' },
        ],
    },
    office: {
        name: '🏢 办公套件',
        description: 'LLM + Agent + Word + Excel + PPT + PDF + Email + Calendar + Memory',
        components: [
            { type: 'llm', size: 5, x: 3, y: 3, apiSettings: { apiBase: '', apiKey: '', model: '', provider: '自定义' } },
            { type: 'agent', size: 6, x: 3, y: 35 },
    ...
    pua_coach: {
        name: '🔥 PUA 高压教练',
        description: 'LLM + Skills Manager(PUA Coach) + Search + Code + Calc + FileOps + Memory',
        components: [
            { type: 'llm', size: 5, x: 3, y: 3, apiSettings: { apiBase: '', apiKey: '', model: '', provider: '自定义' } },
            { type: 'skills_manager', size: 5, x: 3, y: 38 },
        ...
        connections: [
            ...
            { source: 1, sourcePort: 'skm-skill-1', target: 2, targetPort: 'skill-in' },
            { source: 0, sourcePort: 'llm-out', target: 3, targetPort: 'ws-in' },
            { source: 0, sourcePort: 'llm-out', target: 4, targetPort: 'code-in' },
            { source: 0, sourcePort: 'llm-out', target: 5, targetPort: 'calc-in' },
            { source: 0, sourcePort: 'llm-out', target: 6, targetPort: 'fo-in' },
            { source: 8, sourcePort: 'mem-out', target: 0, targetPort: 'llm-mem-in' },
        ],
    },
    full_agent: {
        name: '🤖 全能 Agent',
        description: 'LLM + Agent + Search + Code + Calc + Excel + Word + PDF + Memory + Reflection',
        components: [
            { type: 'llm', size: 6, x: 3, y: 3, apiSettings: { apiBase: '', apiKey: '', model: '', provider: '自定义' } },
            { type: 'agent', size: 6, x: 3, y: 38 },
            { type: 'web_search', size: 3, x: 50, y: 3, toolEnabled: true },
            { type: 'code_executor', size: 3, x: 68, y: 3, toolEnabled: true },
